...
%TODO figures: commas => dots
%TODO anonymize review version
%TODO denote dataset as work in progress and training version as 'v1.0'
...
\vspace*{-2.5mm}
Faster R-CNN consists of two stages, the first being a \ac{RPN} to detect object candidates in the feature maps and the second being classifiers for the candidate regions. While the second stage specifies on the trained brands, the \ac{RPN} will generate candidates that vaguely resemble any of the brands which is the case for many other logos. Thus, it provides an indicator whether a region of the image is a logo or not. The trained \ac{RPN} and the underlying feature extractor network are extracted and employed as a baseline open set logo detector. 

\subsubsection*{Brand Agnostic}
\vspace*{-2.5mm}
The \ac{RPN} strategy is by no means optimal because it obviously has a bias towards the pre-trained brands and also includes a certain amount of false positives.
...
Second, the bounding box regression layers better adapt the bounding boxes to the logos resulting in a more precise localization. 
%Third, the combination of \ac{RPN} and the fully connected classifiers shape a cascade of detectors similarly as in \cite{viola2004}. The \ac{RPN} counts for a weaker classifier, which is faster and allows more false positives. In combination, it is expected to have a lower false positive rate.
...
In all cases, the \acp{CNN} are trained until convergence. Due to the diversity of applied networks and differing dataset sizes, training settings are numerous and optimized in each case with the validation data. Convergence occurs after 200 to 8,000 training iterations with a batch-size between 1 for the Faster R-CNN detector, 32 for the VGG16, 18 for the ResNet101 and 7 for the DenseNet161 training, due to GPU memory limitation.
%Training duration depends on the architecture as well as the amount of training data.
%TODO write text here

\subsection{Detection}
%- train 3 different detectors: multi-class Faster R-CNN (maybe \ac{RPN}), single-class Faster R-CNN, single-class Faster R-CNN + own logos\\
...
The proposed logo detector is first trained on the same public data for comparison. All Faster R-CNN detectors are based on VGG16 network.
